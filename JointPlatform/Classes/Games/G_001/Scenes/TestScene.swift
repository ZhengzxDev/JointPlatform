...
class TestScene:BaseScene{
    ...
    /*private var uiOrigin:CGPoint{
        get{
            return CGPoint(x: -self.size.width/2, y: self.size.height/2)
        }
    }
    
    private var uiRightBottom:CGPoint{
        get{
            return CGPoint(x: self.size.width/2, y: -self.size.height/2)
        }
    }*/
    
    private var airdropSpawnTimer:Timer?
    ...
    override func sceneDidLoad() {
        ...
        backgroundContainer = self.childNode(withName: "background")
    ...
    override func didMove(to view: SKView) {
        ...
        if let backComp = backgroundContainer.entity!.component(ofType: BackLayerMoveComponent.self){
            backComp.setCamera(cameraNode)
            backComp.addLayer(node: backgroundContainer.childNode(withName: "layer_4")!, idx: 0, offset: .zero, moveDelta: 0.8)
            backComp.addLayer(node: backgroundContainer.childNode(withName: "layer_5")!, idx: 1, offset: .zero, moveDelta: 0.6)
            backComp.addLayer(node: backgroundContainer.childNode(withName: "layer_6")!, idx: 2, offset: .zero, moveDelta: 0.5)
            backComp.addLayer(node: backgroundContainer.childNode(withName: "layer_7")!, idx: 3, offset: .zero, moveDelta: 0.4)
            backComp.addLayer(node: backgroundContainer.childNode(withName: "layer_8")!, idx: 4, offset: .zero, moveDelta: 0.3)
            backComp.addLayer(node: backgroundContainer.childNode(withName: "layer_9")!, idx: 5, offset: .zero, moveDelta: 0.2)
            
        }
        addTileMapPhysicsBody()
        //gameController?.playBgm(name: "bgm_fight_test_scene",vol: 0.25)
        GlobalSoundsManager.shared.playBgm(name: "bgm_fight_test_scene", vol: GameUserConfig.global.gameBgmVolume)
        //spawnRandomWeapons()
        
        
        airdropSpawnTimer = Timer.scheduledTimer(timeInterval: 15, target: self, selector: #selector(spawnAirdrop), userInfo: nil, repeats: true)
    ...
    @objc func spawnAirdrop(){
        let craft = StuffDeliverAircraftNode(weapon: WeaponInfo.load(with: "\(Int.random(from: 1, to: 12))")!, targetXPosition: Double.random()*500, scene: self)
        self.addChild(craft)
        self.addTempNode(craft)
    }
    ...
    /*func addTileMapPhysicsBody(){
        ...
        var rects:[CGRect] = []
        let singleTileSize:CGSize = tileMapNode.tileSize
        var colVerticalNeighborRect:[Int:(Int,CGRect)] = [:]
        let tileMapNodeOrigin:CGPoint = CGPoint(x:  -tileMapNode.frame.width/2, y: -tileMapNode.frame.height/2)
        var rectCounter:Int = 0
            ...
            for col in 0 ..< tileMapNode.numberOfColumns{
                ...
                if tileNodeDef.userData?["isEdge"] as? Bool == true{
                    ...
                    if lineRects.isEmpty{
                        ...
                        //when line rects is empty, them col merge has higher priority.
                        
                        var isIncluded:Bool = false
                        //merge vertical rects if needed
                        if let vertNeighbor = colVerticalNeighborRect[col]{
                            if vertNeighbor.1.maxY == tileNodeOrigin.y{
                                //vertical adjacent , then enlarge previous rect
                                rects[vertNeighbor.0].size.height += singleTileSize.height
                                colVerticalNeighborRect[col]!.1 = rects[vertNeighbor.0]
                                isIncluded = true
                            }
                        }
                        ...
                        if !isIncluded{
                            //is not be inclued by previous rect
                            //update this rect as this row's new vertical adjacent.
                            colVerticalNeighborRect[col] = (rectCounter,CGRect(origin: tileNodeOrigin, size: singleTileSize))
                            lineRects.append(CGRect(origin: tileNodeOrigin, size: singleTileSize))
                            rectCounter += 1
                        }

                    }
                    else{
                        //if left side is edge ,then merge it to left.
                        if lineRects.last!.maxX == tileNodeOrigin.x{
                            //horizontal adjacent , then enlarge previous rect
                            lineRects[lineRects.count-1].size.width += singleTileSize.width
                            
                            let lastNodeOrigin = tileMapNodeOrigin.offsetBy(dx: CGFloat(col-1)*singleTileSize.width, dy: CGFloat(row)*singleTileSize.height)
                            if colVerticalNeighborRect[col-1]?.1.origin == lastNodeOrigin{
                                //reset last row's vertical adjacent if last node is recorded as last col's vertical adjacent.
                                colVerticalNeighborRect.removeValue(forKey: col-1)
                            }
                            
                        }
                        else{
                            //not adjacent
                            //col merge has higher priority.
                            var isIncluded:Bool = false
                            //merge vertical rects if needed
                            if let vertNeighbor = colVerticalNeighborRect[col]{
                                if vertNeighbor.1.maxY == tileNodeOrigin.y{
        ...
        //fix bug
        for (idx,bodyBounds) in rects.enumerated(){
            
            guard bodyBounds.height >= bodyBounds.width else { continue }
            
            let uppperDot = bodyBounds.origin.offsetBy(dx: singleTileSize.width/2, dy: bodyBounds.height+singleTileSize.height/2)
            let lowerDot = bodyBounds.origin.offsetBy(dx: singleTileSize.width/2, dy: -singleTileSize.height/2)
            
            /*let n = SKSpriteNode(texture: SKTexture(), color: .blue, size: CGSize(width: 5, height: 5))
            n.position = uppperDot
            n.zPosition = 999
            tileMapNode.addChild(n)
            
            let n1 = SKSpriteNode(texture: SKTexture(), color: .blue, size: CGSize(width: 5, height: 5))
            n1.position = lowerDot
            n1.zPosition = 999
            tileMapNode.addChild(n1)*/
            
            for subRect in rects{
                if subRect.contains(uppperDot){
                    
                    rects[idx].size.height += subRect.height
                }
                else if subRect.contains(lowerDot){
                    rects[idx].size.height += subRect.height
                    rects[idx].origin = rects[idx].origin.offsetBy(dx: 0, dy: -subRect.height)
                }
            }
...
extension TestScene:InGameUIViewDelegate{
